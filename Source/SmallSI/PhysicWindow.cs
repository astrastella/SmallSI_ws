...
namespace SmallSI
{
    internal class PhysicWindow : GameWindow2D
    {
        ...
        private void LoadCircleScene()
        {
            ...
            // Adjust physics settings for better circle behavior
            physicScene.Settings.PositionalCorrectionRate = 0.1f; // Reduce from default 0.2f to minimize jittering
            physicScene.Settings.AllowedPenetration = 2.0f; // Increase from default 1.0f for more stable contacts
            physicScene.Settings.IterationCount = 15; // Increase iterations for better convergence
            
            // Create boundaries (walls and floor) - original size
            physicScene.Bodies.Add(new RigidRectangle(new Vec2D(450f, 540f), new Vec2D(900f, 20f), 0f, float.MaxValue, 0.6f, 0.2f)); // Bottom wall
            physicScene.Bodies.Add(new RigidRectangle(new Vec2D(10f, 275f), new Vec2D(20f, 550f), 0f, float.MaxValue, 0.6f, 0.2f)); // Left wall
            physicScene.Bodies.Add(new RigidRectangle(new Vec2D(890f, 275f), new Vec2D(20f, 550f), 0f, float.MaxValue, 0.6f, 0.2f)); // Right wall
            physicScene.Bodies.Add(new RigidRectangle(new Vec2D(450f, 10f), new Vec2D(900f, 20f), 0f, float.MaxValue, 0.6f, 0.2f)); // Top wall

            // Add a falling circle from the top with very high bounce
            physicScene.Bodies.Add(new RigidCircle(new Vec2D(450f, 100f), 25f, 0.001f, 0.95f, 0.1f)); // Very high restitution, low friction
            
            // Add some static rectangles as obstacles with better physics properties
            physicScene.Bodies.Add(new RigidRectangle(new Vec2D(300f, 400f), new Vec2D(100f, 20f), 0.3f, float.MaxValue, 0.4f, 0.2f));
            physicScene.Bodies.Add(new RigidRectangle(new Vec2D(600f, 300f), new Vec2D(100f, 20f), -0.3f, float.MaxValue, 0.4f, 0.2f));
        }
